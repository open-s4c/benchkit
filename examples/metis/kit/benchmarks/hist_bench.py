...
_DATA_FILE_NAMES = {"test": "3MB.bmp", "bench": "hist-2.6GB.bmp"}


class HistBench(Benchmark):
    ...
    def prebuild_bench(  # pylint: disable=arguments-differ
        ...
        if not os.path.isfile(data_bench_file_path):
            ...
            with (
                open(data_test_file_path, "rb") as f_test,
                open(data_bench_file_path, "wb") as f_bench,
            ):
                chunk = f_test.read()
                for _ in range(887):
                    f_bench.write(chunk)
