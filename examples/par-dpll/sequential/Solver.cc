...
class Solver {
    ...
    bool search(std::vector<int> &solution)
    {
        ...
        int currentVar = 0;
        int i = 1;
        std::vector<int> prop_lits = find_prop_lits(solution,possible_propagators);
        ...
        do {
            ...
            prop_lits = find_prop_lits(solution,possible_propagators);
            ...
            else
            {
                sort(decisionsLeft.begin(),decisionsLeft.end(),[this](int a, int b){
                    if(var_activities[a-1]==var_activities[b-1]) return a < b;
    ...
    std::vector<int> find_prop_lits(std::vector<int> &solution,vector<int> possible_propagators)
    {
