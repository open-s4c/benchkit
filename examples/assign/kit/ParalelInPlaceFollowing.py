...
class ThreadCounterBench(Benchmark):
    ...
    def __init__(
        ...
        self._bench_src_path = build_dir/"build"
    ...
    def parse_output_to_results(  # pylint: disable=arguments-differ
        ...
        **_kwargs,
        ...
        cores = int(run_variables["cores"])
        locks = int(run_variables["locks"])
        result_dict = self._parse_results(log_output=command_output, itterations=itterations, size=size, cores=cores,locks=locks)
        return result_dict
    ...
    def single_run(  # pylint: disable=arguments-differ
        ...
        **_kwargs,
        ...
        build_dir = self._build_dir
        self.platform.comm.shell(
            command=f"./setupEnv.sh",
            current_dir=build_dir,
            output_is_log=True,
        )
        environment = {}

        command = ["./ParallelInPlaceFollowing", "-seed", str(90), "-i", str(itterations), "-s", str(size), "-t", str(cores),"-l", str(locks),
                    "--ctl_file", "/tmp/perf_ctl.fifo", "--ctl_ack_file", "/tmp/perf_ctl_ack.fifo"]
...
def ParallelInPlaceFollowing_campaign(
    name: str = "campaign",
