...
from benchkit.campaign import CampaignIterateVariables, CampaignSuite, Constants
...
class NgapBench(Benchmark):
    ...
    @staticmethod
    def _parse_results(output: str, run_variables: Dict[str, Any]) -> Dict[str, str]:
    ...
    def single_run(  # pylint: disable=arguments-differ
        ...
        **kwargs,
        ...
        command = ["ngap", f"--automata={automata}", f"--input={input}", f"--app-name={app_name}"]

        if algorithm is not None:
            command.extend([f"--algorithm={algorithm}"])
        if input_start_pos is not None:
            command.extend([f"--input-start-pos={input_start_pos}"])
        if input_len is not None:
            command.extend([f"--input-len={input_len}"])
        if split_entire_inputstream_to_chunk_size is not None:
            command.extend(
                [
                    f"--split-entire-inputstream-to-chunk-size="
                    f"{split_entire_inputstream_to_chunk_size}"
                ]
            )
        if group_num is not None:
            command.extend([f"--group-num={group_num}"])
        if duplicate_input_stream is not None:
            command.extend([f"--duplicate-input-stream={duplicate_input_stream}"])
        if unique is not None:
            command.extend([f"--unique={unique}"])
        if unique_frequency is not None:
            command.extend([f"--unique-frequency={unique_frequency}"])
        if use_soa is not None:
            command.extend([f"--use-soa={use_soa}"])
        if result_capacity is not None:
            command.extend([f"--result-capacity={result_capacity}"])
        if use_uvm is not None:
            command.extend([f"--use-uvm={use_uvm}"])
        if data_buffer_fetch_size is not None:
            command.extend([f"--data-buffer-fetch-size={data_buffer_fetch_size}"])
        if add_aan_start is not None:
            command.extend([f"--add-aan-start={add_aan_start}"])
        if add_aas_interval is not None:
            command.extend([f"--add-aas-interval={add_aas_interval}"])
        if active_threshold is not None:
            command.extend([f"--active-threshold={active_threshold}"])
        if precompute_cutoff is not None:
            command.extend([f"--precompute-cutoff={precompute_cutoff}"])
        if precompute_depth is not None:
            command.extend([f"--precompute-depth={precompute_depth}"])
        if compress_prec_table is not None:
            command.extend([f"--compress-prec-table={compress_prec_table}"])
        if pc_use_uvm is not None:
            command.extend([f"--pc-use-uvm={pc_use_uvm}"])
        if report_off is not None:
            command.extend([f"--report-off={report_off}"])
        if remove_degree is not None:
            command.extend([f"--remove-degree={remove_degree}"])
        if quit_degree is not None:
            command.extend([f"--quit-degree={quit_degree}"])
        if max_nfa_size is not None:
            command.extend([f"--max-nfa-size={max_nfa_size}"])

        if adaptive_aas is not None:
            command.extend([f"--adaptive-aas={adaptive_aas}"])
        if block_size is not None:
            command.extend([f"--block-size={block_size}"])
        if motivate_worklist_length is not None:
            command.extend([f"--motivate-worklist-length={motivate_worklist_length}"])
        if num_state_per_group is not None:
            command.extend([f"--num-state-per-group={num_state_per_group}"])
        if only_exec_cc_with_state_id is not None:
            command.extend([f"--only-exec-cc-with-state-id={only_exec_cc_with_state_id}"])
        if only_exec_ccid is not None:
            command.extend([f"--only-exec-ccid={only_exec_ccid}"])
        if output_file is not None:
            command.extend([f"--output-file={output_file}"])
        if padding is not None:
            command.extend([f"--padding={padding}"])
        if report_filename is not None:
            command.extend([f"--report-filename={report_filename}"])
        if try_adaptive_aas is not None:
            command.extend([f"--try-adaptive-aas={try_adaptive_aas}"])
        if tuning is not None:
            command.extend([f"--tuning={tuning}"])
...
def get_breakdown_configs():
    return [
        (
            "./configs/app_spec_ngap_new_quickvalidation_part1",
            "./configs/exec_config_ngap_groups_design_NAP",
        ),
        (
            "./configs/app_spec_ngap_new_quickvalidation_part2",
            "./configs/exec_config_ngap_groups_design_NAP",
        ),
        (
            "./configs/app_spec_ngap_new_quickvalidation_part3",
...
def main():
    ...
    for app, config in breakdown_configs:
        breakdown_campaigns.append(
            ngap_campaign(
                app_file=app, config_file=config, name_prefix="breakdown", enable_data_dir=True
            )
        )
