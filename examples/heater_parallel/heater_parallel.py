...
class HeaterParBench(Benchmark):
    """Benchmark object for Heater Parallel benchmark."""
    ...
    def __init__(
        ...
        if build_dir is None:
            ...
            self._tmpdb_dir = "/tmp/benchkit_heater_parallel"
        else:
            self._build_dir = self._bench_src_path / build_dir
            self._tmpdb_dir = self._build_dir / "tmp" / "benchkit_heater_parallel"
    ...
    def _parse_results(
        ...
        match = re.search(r"Counter\s*=\s*(\d+)", output)
        if not match:
    ...
    def prebuild_bench(self, **_kwargs) -> None:
        ...
        self.platform.comm.shell(
            command=f"gcc -O3 -o heater {self._bench_src_path / 'heater.c'} -pthread -lnuma",
