...
class PhoronixTestProfileBench(Benchmark):
    ...
    def _get_executable_name(self):
        ...
        test_def_defined = self._profile.test_def is not None
        test_info_exec_defined = self._profile.test_def.test_information.executable is not None
        if test_def_defined and test_info_exec_defined:
    ...
    def get_run_var_names(self) -> List[str]:
        if self._profile.test_def is None or self._profile.test_def.test_settings is None:
            return []
        result = [option.identifier for option in self._profile.test_def.test_settings.options]
        return result
        ...
        if test_def is not None and test_def.test_settings is not None:
            if test_def.test_settings.options is not None:
                for option in test_def.test_settings.options:
                    ...
                    if option.argument_prefix is not None:
                        comm += option.argument_prefix
                    ...
                    if option.argument_postfix is not None:
                        comm += option.argument_postfix
                    command.append(comm)
        ...
        for index, results_parser in enumerate(self._profile.results_def.results_parsers):
            ...
            if results_parser.delete_output_before is not None:
                command_output = command_output[
                    command_output.find(results_parser.delete_output_before) :
                ]
            if results_parser.delete_output_after is not None:
                command_output = command_output[
                    : command_output.find(results_parser.delete_output_after)
                ]
            ...
            regex = r"#_(.*?)_#"
            result_keys = [m.group(1) for m in re.finditer(regex, output_template, re.MULTILINE)]
            ...
            for line in command_output.splitlines():
                ...
                for index in range(len(parts)):
                    ...
                    if results_parser.strip_result_postfix is not None and result.endswith(
                        results_parser.strip_result_postfix
                    ):
                    ...
                    if results_parser.divide_result_by is not None:
                        try:
                            result = float(result) / results_parser.divide_result_by
                        except Exception:
                            pass
                    if results_parser.divide_result_divisor is not None:
                        try:
                            result = float(result) // results_parser.divide_result_divisor
                        except Exception:
                            pass
                    if results_parser.multiply_result_by is not None:
                        try:
                            result = float(result) * results_parser.multiply_result_by
                        except Exception:
                            pass
