...
from benchkit.campaign import CampaignSuite, CampaignCartesianProduct
...
from typing import Tuple, Iterable, Tuple
...
def build_locks(platform: Platform) -> Tuple[Path, Path]:
    ...
    platform.comm.shell(command="git checkout -- include/vsync/atomic/internal/arm64.h", current_dir=vsync_dir)
...
def get_baseline_campaign() -> CampaignCartesianProduct:
    return get_campaign(mutex_constant="pthread_mutex", shared_libs=[],)


def get_caslock_campaign(build_path: Path) -> CampaignCartesianProduct:
    caslocklib_path = (build_path/"libcaslock.so").resolve()
    return get_campaign(mutex_constant="CAS lock (std atomic)", shared_libs=[PrecompiledSharedLib(path=caslocklib_path, env_vars=None)],)


def get_vcaslock_campaign(build_path: Path) -> CampaignCartesianProduct:
    vcaslocklib_path = (build_path / "libvcaslock.so").resolve()
    return get_campaign(mutex_constant="CAS lock (VSync)", shared_libs=[PrecompiledSharedLib(path=vcaslocklib_path, env_vars=None)],)


def get_vcaslock_prefetch_campaign(build_path: Path) -> CampaignCartesianProduct:
    vcaslocklib_path = (build_path / "libvcaslock.so").resolve()
    return get_campaign(mutex_constant="CAS lock (VSync, prefetch)", shared_libs=[PrecompiledSharedLib(path=vcaslocklib_path, env_vars=None)],)
