...
class CommandProcess():
    def __init__(self,
                 ...
                 output:Output, timeout:Optional[int],
                 success_code:int = 0,
        ...
        self.ignore_exit_codes:Iterable[int] = (success_code,) if ignore_exit_codes is None else ignore_exit_codes
    ...
    @staticmethod
    def wait_func(subprocess:Popen[bytes],queue:Queue[Tuple[int,Optional[Exception]]],timeout:Optional[int],ignore_exit_codes:Iterable[int]) -> None:
        try:
    ...
    def __wait_async(self) -> Thread:
        waiting_thread = Thread(target=self.wait_func,args=(self.__popen_object,self.__retcode_queue,self.__timeout,self.ignore_exit_codes))
